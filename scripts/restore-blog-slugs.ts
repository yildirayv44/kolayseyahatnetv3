...
async function restoreBlogSlugs() {
  console.log('🔄 Starting blog slug restoration...\n');
  ...
  if (!taxonomies || taxonomies.length === 0) {
    console.log('✅ No blog slugs to restore (all slugs are already in old format)');
    return;
  }

  console.log(`📊 Found ${taxonomies.length} blog slugs with new format\n`);
...
restoreBlogSlugs()
  .then(() => {
    console.log('\n✅ Blog slug restoration completed!');
    process.exit(0);
  })
  .catch((error) => {
    console.error('\n❌ Script failed:', error);
